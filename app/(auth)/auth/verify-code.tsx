...
export default function VerifyCodeScreen() {
  ...
  useEffect(() => {
    // Entrada secuencial: header → card
    Animated.sequence([
      Animated.parallel([
        Animated.timing(fadeHdr,  { toValue: 1, duration: 400, useNativeDriver: true }),
        Animated.timing(slideHdr, { toValue: 0, duration: 260, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
      ]),
      Animated.parallel([
        Animated.timing(fadeCard,  { toValue: 1, duration: 350, useNativeDriver: true }),
        Animated.timing(slideCard, { toValue: 0, duration: 240, easing: Easing.out(Easing.cubic), useNativeDriver: true }),
      ]),
    ]).start(() => {
  ...
  return (
    <ThemedView style={[styles.container, { backgroundColor: bg }]}>
      ...
      <KeyboardAvoidingView behavior={Platform.OS === 'ios' ? 'padding' : undefined} style={styles.kav}>
        ...
        <Animated.View style={[styles.header, { opacity: fadeHdr, transform: [{ translateY: slideHdr }] }]}>
          <ThemedText style={styles.brand}>Verificar código</ThemedText>
        ...
        <Animated.View
          style={[
            styles.card,
            { backgroundColor: surface, borderColor: border, opacity: fadeCard, transform: [{ translateY: slideCard }] },
          ]}
        >
          ...
          {errors.code && <ThemedText style={[styles.errorText, { color: errorColor }]}>{errors.code.message}</ThemedText>}
      ...
      <Animated.View pointerEvents={busy ? 'auto' : 'none'} style={[StyleSheet.absoluteFill, { opacity: overlayOpacity }]}>
        {busy && <Loader visible variant="overlay" message={busyMsg} backdropOpacity={0.45} />}
      </Animated.View>
    </ThemedView>
  );
}
